...
def get_user(uid):
    if uid not in users:
        users[uid] = {"game":None,"product":None,"price":None,"username":None,"payment":None,"order_id":None,"brawl_pass_type":None,"brawl_pass_waiting":False,"receipt_waiting_id":False,"receipt_order_message_id":None,"receipt_accepted":False,"refund_waiting":False,"refund_method":None,"refund_operator":None,"refund_details_ready":False}
    return users[uid]
...
async def finalize(uid,status,client_text):
    ...
    u["receipt_order_message_id"]=None
    u["receipt_waiting_id"]=False
    u["receipt_accepted"]=False
    u["refund_waiting"]=False
    u["refund_details_ready"]=False
...
async def game(c):
    g=c.data.split(":",1)[1]; u=get_user(c.from_user.id); u.update({"game":g,"product":None,"price":None,"payment":None,"brawl_pass_type":None,"brawl_pass_waiting":False})
...
async def photo(m):
    ...
    if u.get("brawl_pass_waiting"):
        ...
        u["pass_admin_message_id"]=sent.message_id; await m.answer("✅ Screenshot-ը ստացվեց։ Ադմինը կընտրի ճիշտ գինը։",reply_markup=main_kb()); return
    if u.get("payment")!="receipt_pending": return
    u["payment"]="receipt_sent"; u["order_id"]=f"{m.from_user.id}-{m.message_id}"; u["username"]=m.from_user.username; u["receipt_accepted"]=False
    ...
    u["receipt_order_message_id"]=sent.message_id; u["receipt_waiting_id"]=True
    await m.answer("💎 <b>Չեկը ստացվեց։</b> ❤️‍🔥\n\n🆔 Հիմա ուղարկիր քո ID-ն։",parse_mode="HTML")

@dp.message(F.text)
async def text(m):
    u=get_user(m.from_user.id)
    if u.get("refund_waiting") and u.get("refund_method") in {"phone","card"} and u.get("refund_details_ready") is False and u.get("refund_operator"):
        u["refund_details_ready"]=True; u["refund_details"]=m.text.strip(); await notify(ADMIN_ID,f"💸 <b>Տվյալները վերադարձի համար</b>\n\n🆔 User ID՝ <code>{m.from_user.id}</code>\n📦 {escape(u.get('product') or '')}\n💰 {fmt(u.get('price') or 0)} ֏\n📌 Եղանակ՝ <b>{escape(u['refund_method'])}</b>\n📞 Օպերատոր՝ <b>{escape(u.get('refund_operator') or '-')}</b>\n📝 Տվյալներ՝ <code>{escape(m.text.strip())}</code>",admin_kb(m.from_user.id,u.get("receipt_accepted",False),True)); await m.answer("✅ Տվյալները ստացվեցին։ Սպասիր վերադարձի հաստատմանը։"); return
    if u.get("refund_waiting") and u.get("refund_method")=="card" and not u.get("refund_operator"):
        u["refund_details_ready"]=True; u["refund_details"]=m.text.strip(); await notify(ADMIN_ID,f"💳 <b>Քարտի տվյալները վերադարձի համար</b>\n\n🆔 <code>{m.from_user.id}</code>\n📦 {escape(u.get('product') or '')}\n💰 {fmt(u.get('price') or 0)} ֏\n📝 <code>{escape(m.text.strip())}</code>",admin_kb(m.from_user.id,u.get("receipt_accepted",False),True)); await m.answer("✅ Քարտի տվյալները ստացվեցին։ Սպասիր վերադարձի հաստատմանը։"); return
...
async def cash(c):
    u=get_user(c.from_user.id); u["payment"]="cash"; await c.message.edit_text(cash_text(u),reply_markup=receipt_kb(),parse_mode="HTML"); await c.answer()

@dp.callback_query(F.data=="payment:done")
async def done(c):
    u=get_user(c.from_user.id); u["payment"]="receipt_pending"; await c.message.edit_text("🧾 <b>Ուղարկիր չեկի նկարը</b>\n\n📸 Ուղարկիր վճարման չեկի լուսանկարը։",reply_markup=back_kb("back:payment"),parse_mode="HTML"); await c.answer()
...
async def receipt(c):
    ...
    if action=="accept":
        u["receipt_accepted"]=True; u["payment"]="receipt_accepted"; u["receipt_order_message_id"]=c.message.message_id
    ...
    if action=="refund":
        u["refund_waiting"]=True; u["refund_method"]=None; u["refund_operator"]=None; u["refund_details_ready"]=False
        await notify(uid,"💸 <b>Դրամի վերադարձ</b>\n\nԸնտրիր, թե որտեղ պետք է կատարվի վերադարձը։",refund_method_kb()); await c.answer("💸 Հաճախորդին ուղարկվեց վերադարձի ընտրությունը։"); return
...
async def rphone(c):
    u=get_user(c.from_user.id); u["refund_method"]="phone"; await c.message.edit_text("📱 <b>Ընտրիր քո բջջային օպերատորը</b>։",reply_markup=operators_kb(),parse_mode="HTML"); await c.answer()

@dp.callback_query(F.data=="refundclient:card")
async def rcard(c):
    u=get_user(c.from_user.id); u["refund_method"]="card"; u["refund_operator"]=None; await c.message.edit_text("💳 <b>Ուղարկիր քարտի համարը</b>։\n\n⚠️ Մի ուղարկիր CVV/CVC, PIN կամ այլ գաղտնի կոդ։",reply_markup=back_kb("refundclient:back"),parse_mode="HTML"); await c.answer()

@dp.callback_query(F.data.startswith("refundclient:operator:"))
async def roperator(c):
    u=get_user(c.from_user.id); op=c.data.split(":",2)[2]; u["refund_operator"]=op
...
async def rback(c):
    u=get_user(c.from_user.id); u["refund_method"]=None; u["refund_operator"]=None; u["refund_details_ready"]=False; await c.message.edit_text("💸 <b>Ընտրիր վերադարձի եղանակը</b>։",reply_markup=refund_method_kb(),parse_mode="HTML"); await c.answer()

@dp.callback_query(F.data=="contact:open")
async def contact(c):
    await c.answer("📩 Կապվեք մեզ հետ՝ գրեք հաղորդագրություն։",show_alert=True)
