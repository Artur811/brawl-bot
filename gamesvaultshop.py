...
def refund_keyboard(user_id: int):
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="💳 Վերադարձ քարտին", callback_data=f"refund:card:{user_id}")],
        [InlineKeyboardButton(text="📱 Վերադարձ հեռախոսահամարին", callback_data=f"refund:phone:{user_id}")],
    ])


def refund_done_keyboard(user_id: int, method: str):
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ Վերադարձը կատարված է", callback_data=f"refund:done:{method}:{user_id}")]
    ])
...
async def photo_router(message: Message):
    ...
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(text="✅ Ընդունել չեկը", callback_data=f"order:accept:{message.from_user.id}"),
            InlineKeyboardButton(text="❌ Մերժել չեկը", callback_data=f"order:reject:{message.from_user.id}"),
        ],
        [InlineKeyboardButton(text="💸 Վերադարձ կատարել", callback_data=f"order:refund:{message.from_user.id}")],
    ])

    await bot.send_photo(
        chat_id=target,
        photo=message.photo[-1].file_id,
        caption=order_caption(message.from_user.id, user, "🟡 <b>Սպասում է չեկի ստուգման</b>"),
...
async def support_channel_reply(message: Message):
    if not SUPPORT_CHANNEL_ID or str(message.chat.id) != str(SUPPORT_CHANNEL_ID):
        return
    ...
    try:
        if message.text:
            await bot.send_message(
                user_id,
                f"📩 <b>Պատասխան Games Vault Shop-ից</b>\n\n{escape(message.text)}",
                parse_mode="HTML",
            )
        elif message.photo:
            await bot.send_photo(
                ...
                caption="📩 <b>Պատասխան Games Vault Shop-ից</b>",
                parse_mode="HTML",
            )
    except Exception:
...
async def payment_done(callback: CallbackQuery):
    ...
    user["order_id"] = f"{callback.from_user.id}-{int(asyncio.get_running_loop().time())}"
    await callback.message.edit_text(
        "🧾 <b>Ուղարկիր վճարման չեկը</b>\n\n"
        "Ուղարկիր Telcell-ի վճարման չեկի նկարը այս չաթում։\n\n"
        "❗ Չեկը պետք է ամբողջությամբ տեսանելի լինի։",
...
async def order_accept(callback: CallbackQuery):
    ...
    try:
        await callback.message.edit_caption(
            caption=order_caption(user_id, user, "✅ <b>Չեկը ընդունված է</b>"),
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[[
                InlineKeyboardButton(text="💸 Վերադարձ կատարել", callback_data=f"order:refund:{user_id}")
            ]]),
        )
...
async def order_reject(callback: CallbackQuery):
    ...
    await bot.send_message(
        user_id,
        "❌ <b>Չեկը մերժված է։</b>\n\nՍտուգիր վճարումը և անհրաժեշտության դեպքում ուղարկիր նոր չեկ։",
        parse_mode="HTML",
    )
    try:
        await callback.message.edit_caption(
            caption=order_caption(user_id, user, "❌ <b>Չեկը մերժված է</b>"),
            parse_mode="HTML",
            reply_markup=None,
        )
    except TelegramBadRequest:
...
async def order_refund(callback: CallbackQuery):
    ...
    user_id = int(callback.data.rsplit(":", 1)[1])
    await callback.message.edit_caption(
        caption=(callback.message.caption or "") + "\n\n💸 <b>Ընտրվեց վերադարձի եղանակը</b>",
        parse_mode="HTML",
        reply_markup=refund_keyboard(user_id),
    )
    await callback.answer()
...
async def refund_card(callback: CallbackQuery):
    ...
    await callback.message.edit_caption(
        caption=(callback.message.caption or "") + "\n\n💳 <b>Վերադարձ՝ քարտին</b>\n🟡 Սպասում է կատարմանը",
        parse_mode="HTML",
        reply_markup=refund_done_keyboard(user_id, "card"),
    )
...
async def refund_phone(callback: CallbackQuery):
    ...
    await bot.send_message(
        ...
        "⚠️ Եթե գումարը վերադարձվում է հեռախոսահամարին, այն կավելանա հեռախոսի <b>հաշվեկշռին / զանգերի բալանսին</b>։ "
        "Այն չի փոխանցվի որպես քարտային կամ կանխիկ գումար.",
        parse_mode="HTML",
    )
    await callback.message.edit_caption(
        caption=(callback.message.caption or "") + "\n\n📱 <b>Վերադարձ՝ հեռախոսահամարին</b>\n🟡 Սպասում է կատարմանը",
        parse_mode="HTML",
        reply_markup=refund_done_keyboard(user_id, "phone"),
    )
