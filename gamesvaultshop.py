...
def get_user(user_id: int):
    if user_id not in users:
        users[user_id] = {
            ...
            "brawl_pass_order_message_id": None,
        }
...
def brawl_pass_prompt(pass_type: str):
    ...
    return (
        ...
        "⚠️ «Լավագույն ակցիա» գրությունը ինքն իրեն որոշիչ չէ։ Կարևոր է հենց քո ընտրած Pass-ի գինը։"
    )
...
async def text_router(message: Message):
    ...
    if user.get("receipt_waiting_id"):
        ...
        await message.answer(
            ...
            "⚡ Վճարումը ստուգելուց հետո ձեր պատվերը կկատարվի։",
    ...
    user["support_waiting"] = False
    await message.answer(
        "✅ Հաղորդագրությունը ստացվեց։ Մենք կպատասխանենք շուտով։",
        reply_markup=main_menu_keyboard(),
    )
...
async def photo_router(message: Message):
    ...
    if user.get("brawl_pass_waiting") and user.get("brawl_pass_type"):
        ...
        caption = (
            ...
            "📸 Ստուգիր screenshot-ը և ընտրիր ներքևի ճիշտ գինը։"
        )
        ...
        await message.answer(
            ...
            "👨‍💼 Ադմինը կստուգի գինը և կընտրի ճիշտ տարբերակը։",
            parse_mode="HTML",
        )
        return
    ...
    if user.get("support_waiting") and SUPPORT_CHANNEL_ID:
        ...
        user["support_waiting"] = False
        await message.answer(
            "✅ Նկարը ստացվեց։ Մենք կպատասխանենք շուտով։",
            reply_markup=main_menu_keyboard(),
        )
        return
    ...
    caption = order_caption(
        ...
        user,
        "⏳ Չեկը ստացվեց։ Սպասում ենք հաճախորդի ID-ին։",
    )
    ...
    try:
        await callback.message.edit_caption(
            caption=(
                ...
                f"💰 Ընտրված գին՝ <b>{fmt_price(price)} ֏</b>\n"
                f"🆔 ID՝ <code>{user_id}</code>\n\n"
                "✅ Ադմինը ընտրեց գինը։"
            ),
            reply_markup=None,
...
async def payment_done(callback: CallbackQuery):
    user = get_user(callback.from_user.id)
    if not user.get("product") or not user.get("price"):
        await callback.answer("❌ Սначала дождитесь подтверждения цены։", show_alert=True)
        return
