...
def verification_text(kind):
    if kind == 'email':
        return ('📧 <b>2FA — E-mail</b>\n\n'
                ...
                '🟢 Երբ ավարտես հաստատումը, պարզապես գրիր՝ <b>«Պատրաստ է»</b>։')
...
async def receipt_action(callback: CallbackQuery):
    ...
    if action == 'ask_ready':
        ...
        await save_state()
        await notify(uid,
            '🔁 <b>Խնդրում ենք կրկին կատարել հաստատումը և վերջում գրել «Պատրաստ է»։</b>\n\n'
            '⚠️ Կոդը կամ գաղտնաբառը մեզ մի ուղարկիր։')
        try:
            await callback.message.edit_caption(
