...
CATALOG = {
    ...
    "brawlstars": {"name": "⭐ Brawl Stars", "items": [("30 Gems", 800), ("80 Gems", 1600), ("170 Gems", 3000), ("360 Gems", 5300), ("950 Gems", 13000), ("Brawl Pass", 2500), ("Brawl Pass+", 3400), ("Պրոգրես Brawl Pass → Brawl Pass+", 1800), ("Pro Pass", 12800)]},
...
def blank_user():
    return {"game": None, "product": None, "price": None, "payment": None, "username": None, "order_id": None, "game_id": None, "receipt_file_id": None, "receipt_waiting": False, "game_id_waiting": False, "game_id_checked": False, "receipt_accepted": False, "verification_type": None, "verification_done": False, "order_message_id": None, "order_chat_id": None, "support_waiting": False, "refund_waiting": False, "refund_method": None, "refund_details": None, "status": "Նոր պատվեր"}
...
def pass_price_kb():
    return kb([[InlineKeyboardButton(text="💰 2 500 ֏", callback_data="passprice:2500"), InlineKeyboardButton(text="💰 3 400 ֏", callback_data="passprice:3400")], [InlineKeyboardButton(text="⬅️ Հետ", callback_data="back:game:brawlstars")]])

def pass_plus_price_kb():
    return kb([[InlineKeyboardButton(text="💰 3 400 ֏", callback_data="passprice:3400"), InlineKeyboardButton(text="💰 4 800 ֏", callback_data="passprice:4800")], [InlineKeyboardButton(text="⬅️ Հետ", callback_data="back:game:brawlstars")]])
...
def admin_id_kb(uid):
    return kb([[InlineKeyboardButton(text="❌ Սխալ ID / Username", callback_data=f"id:reject:{uid}"), InlineKeyboardButton(text="✅ ID / Username ճիշտ է", callback_data=f"id:accept:{uid}")]])

def admin_receipt_kb(uid):
    return kb([[InlineKeyboardButton(text="❌ Սխալ կամ վատ տեսանելի screenshot", callback_data=f"receipt:reject:{uid}")], [InlineKeyboardButton(text="✅ Հաստատել չեկը", callback_data=f"receipt:accept:{uid}")], [InlineKeyboardButton(text="📦 Հաստատել պատվերը", callback_data=f"order:confirm:{uid}")], [InlineKeyboardButton(text="💸 Վերադարձ", callback_data=f"refund:{uid}")]])

def admin_verify_kb(uid):
    return kb([[InlineKeyboardButton(text="📱 Այլ սարքով հաստատում", callback_data=f"verify:device:{uid}")], [InlineKeyboardButton(text="🔐 Authenticator", callback_data=f"verify:auth:{uid}")], [InlineKeyboardButton(text="📧 E-mail", callback_data=f"verify:email:{uid}")], [InlineKeyboardButton(text="⬅️ Հետ", callback_data=f"admin:receipt:{uid}")]])
...
async def create_or_replace_order(uid):
    u = get_user(uid)
    if not u.get("receipt_file_id"): return
    chat_id = int(ORDER_CHANNEL_ID) if ORDER_CHANNEL_ID else ADMIN_ID
    try:
        if u.get("order_message_id") and u.get("order_chat_id"):
            await bot.edit_message_media(chat_id=u["order_chat_id"], message_id=u["order_message_id"], media=InputMediaPhoto(media=u["receipt_file_id"], caption=order_caption(uid), parse_mode="HTML"), reply_markup=admin_id_kb(uid))
        else:
            sent = await bot.send_photo(chat_id, u["receipt_file_id"], caption=order_caption(uid), parse_mode="HTML", reply_markup=admin_id_kb(uid))
...
def new_order(u, message):
    ...
    u["status"] = "Սպասվում է վճարման ապացույց"
...
async def choose_game(c: CallbackQuery):
    ...
    u = get_user(c.from_user.id); u["game"] = game; u["product"] = u["price"] = None
...
async def choose_product(c: CallbackQuery):
    ...
    u = get_user(c.from_user.id); u.update(game=game, product=name, price=price, game_id=None, game_id_checked=False, receipt_file_id=None, receipt_waiting=False, game_id_waiting=False)
    await save_state(); await safe_answer(c)
    if name == "Brawl Pass": await c.message.edit_text("🎫 <b>Brawl Pass</b>\n\nԸնտրիր քո հաշվի տարբերակը։", parse_mode="HTML", reply_markup=pass_price_kb()); return
    if name == "Brawl Pass+": await c.message.edit_text("⭐ <b>Brawl Pass+</b>\n\nԸնտրիր քո հաշվի տարբերակը։", parse_mode="HTML", reply_markup=pass_plus_price_kb()); return
...
async def choose_pass_price(c: CallbackQuery):
    price = int(c.data.split(":")[1]); u = get_user(c.from_user.id)
    if u.get("product") not in {"Brawl Pass", "Brawl Pass+"}: await safe_answer(c, "Ընտրությունը կորել է"); return
    u["price"] = price; await save_state(); await safe_answer(c)
    await c.message.edit_text(f"📦 <b>{escape(u['product'])}</b>\n\n💰 Գին՝ <b>{fmt(price)} ֏</b>\n\nՇարունակե՞նք գնումը։", parse_mode="HTML", reply_markup=product_kb(u["game"]))

@dp.callback_query(F.data == "buy:confirm")
async def buy_confirm(c: CallbackQuery):
    u = get_user(c.from_user.id)
    if not u.get("game") or not u.get("product") or not u.get("price"): await safe_answer(c, "Ընտրիր ապրանքը"); return
...
async def payment_card(c: CallbackQuery):
    ...
    u["payment"] = "Քարտ"; u["receipt_waiting"] = True; new_order(u, c.message); await save_state(); await safe_answer(c)
...
async def payment_telcell(c: CallbackQuery):
    u = get_user(c.from_user.id); u["payment"] = "Telcell"; u["receipt_waiting"] = True; new_order(u, c.message); await save_state(); await safe_answer(c)
...
async def receipt_photo(message: Message):
    ...
    u["receipt_file_id"] = message.photo[-1].file_id; u["receipt_accepted"] = False; u["status"] = "Սպասվում է Game ID / Username"; u["game_id_waiting"] = True; u["receipt_waiting"] = False; new_order(u, message)
    await create_or_replace_order(message.from_user.id)
    await message.answer("📸 Screenshot-ը ստացվեց։\n\n🎯 Հիմա ուղարկիր քո <b>Game ID / Username</b>։", parse_mode="HTML", reply_markup=back_main_kb())
...
async def receipt_reject(c: CallbackQuery):
    if c.from_user.id != ADMIN_ID: await safe_answer(c, "Մուտքը արգելված է"); return
    uid = int(c.data.split(":")[2]); u = get_user(uid); u["receipt_waiting"] = True; u["receipt_accepted"] = False; u["status"] = "Սպասվում է նոր screenshot"; await save_state(); await send_client(uid, "❌ <b>Սխալ կամ վատ տեսանելի screenshot</b>\n\nՈւղարկիր նոր screenshot։ Հին պատվերը չի կրկնվի։"); await update_admin_order(uid, admin_receipt_kb(uid)); await safe_answer(c, "Սպասվում է նոր screenshot")
...
async def verification(c: CallbackQuery):
    ...
    text = {"device": "📱 <b>2FA — այլ սարքով հաստատում</b>\n\nՀաստատիր միայն այն մուտքը, որը դու ես սկսել արդեն մուտք գործած սարքում։", "auth": "🔐 <b>2FA — Authenticator</b>\n\nՕգտագործիր Authenticator-ի կոդը միայն Roblox-ի պաշտոնական էջում։ Կոդը մեզ չի ուղարկվում։", "email": "📧 <b>2FA — E-mail</b>\n\nՕգտագործիր E-mail կոդը միայն Roblox-ի պաշտոնական էջում։ Կոդը մեզ չի ուղարկվում։"}[kind]
    await send_client(uid, text); await update_admin_order(uid, admin_complete_kb(uid)); await safe_answer(c, "2FA-ի եղանակը ընտրվեց")
