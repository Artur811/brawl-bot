...
# ⭐ ТОЛЬКО ДЛЯ BRAWL PASS - цены для админа
BRAWL_PASS_PRICES = {
...
def admin_id_kb(uid): 
    return kb([
        [InlineKeyboardButton(text="❌ Սխալ ID / Username", callback_data=f"id:reject:{uid}"),
         InlineKeyboardButton(text="✅ ID / Username ճիշտ է", callback_data=f"id:accept:{uid}")]
    ])

def admin_password_kb(uid):
    return kb([
        [InlineKeyboardButton(text="❌ Սխալ պասսվորդ", callback_data=f"pass:reject:{uid}"),
         InlineKeyboardButton(text="✅ Պասսվորդ ճիշտ է", callback_data=f"pass:accept:{uid}")]
    ])

def admin_receipt_kb(uid): 
    return kb([
        [InlineKeyboardButton(text="❌ Սխալ կամ վատ տեսանելի screenshot", callback_data=f"receipt:reject:{uid}")],
...
async def photo_input(message: Message):
    ...
    u["status"] = "Սպասվում է Game ID / Username"
    u["game_id_waiting"] = True
    u["receipt_waiting"] = False
    new_order(u, message)
    
    await create_or_replace_order(uid, file_id, admin_id_kb(uid))
    await message.answer(
...
async def text_input(message: Message):
    ...
    if u.get("refund_waiting"):
        ...
        u["status"] = "Սպասվում է վերադարձի մշակում"
        await save_state()
    ...
    if u.get("game_id_waiting"):
        ...
        await save_state()
        await update_admin_order(uid, admin_password_kb(uid))
        await message.answer(
    ...
    if u.get("game_password_waiting"):
        ...
        u["status"] = "Սպասվում է ID-ի և պասսվորդի ստուգում"
        await save_state()
        await update_admin_order(uid, admin_password_kb(uid))
        await message.answer(
...
async def bp_set_price(c: CallbackQuery):
    ...
    u["brawl_pass_rejected"] = False
    u["status"] = f"Գինը ընտրված է՝ {fmt(price)} ֏ — սպասվում է վճարում"
    await save_state()
...
async def bp_reject(c: CallbackQuery):
    ...
    u["status"] = f"Սպասվում է նոր {u['product']} screenshot"
    await save_state()
...
async def admin_id_action(c: CallbackQuery):
    ...
    _, action, uid_s = c.data.split(":")
    uid = int(uid_s)
    u = get_user(uid)
    
    if action == "reject":
        u["game_id_waiting"] = True
        u["game_id_checked"] = False
        u["game_id"] = None
        ...
        await update_admin_order(uid, admin_id_kb(uid))
        await safe_answer(c, "Սպասվում է նոր ID")
    else:
        u["game_id_checked"] = True
        u["status"] = "ID / Username-ը հաստատված է — սպասվում է պասսվորդ"
        await save_state()
        await update_admin_order(uid, admin_password_kb(uid))
        await send_client(uid, "✅ ID / Username-ը հաստատվեց։\n\n🔑 Այժմ սպասիր պասսվորդի ստուգմանը։")
        await safe_answer(c, "ID հաստատվեց")

@dp.callback_query(F.data.startswith("pass:"))
async def admin_password_action(c: CallbackQuery):
    if c.from_user.id != ADMIN_ID:
        await safe_answer(c, "Մուտքը արգելված է")
        return
    ...
    if action == "reject":
        ...
        u["status"] = "ID-ն և պասսվորդը հաստատված են — ստուգիր չեկը"
        await save_state()
        await update_admin_order(uid, admin_receipt_kb(uid))
        await send_client(uid, "✅ Պասսվորդը հաստատվեց։\n\nԱյժմ ստուգվում է վճարումը։")
        await safe_answer(c, "Պասսվորդ հաստատվեց")
...
async def receipt_reject(c: CallbackQuery):
    ...
    u["receipt_waiting"] = True
    u["receipt_accepted"] = False
    u["status"] = "Սպասվում է նոր վճարման screenshot"
    await save_state()
    await send_client(uid, "❌ <b>Վճարման screenshot-ը սխալ է կամ վատ տեսանելի</b>\n\nՈւղարկիր նոր screenshot։ Հին պատվերը չի կրկնվի։")
    await update_admin_order(uid, admin_receipt_kb(uid))
    await safe_answer(c, "Սպասվում է նոր screenshot")
...
async def receipt_accept(c: CallbackQuery):
    ...
    if not u.get("game_id_checked"):
        await safe_answer(c, "Սկզբում հաստատիր ID / Username-ը")
        return
    
    if not u.get("game_password"):
        await safe_answer(c, "Սկզբում հաստատիր պասսվորդը")
        return
    
    u["receipt_accepted"] = True
    u["status"] = "Չեկը հաստատված է — սպասվում է պատվերի հաստատում"
    await save_state()
    await update_admin_order(uid, admin_receipt_kb(uid))
    await send_client(uid, "✅ Վճարման screenshot-ը հաստատվեց։")
...
async def order_confirm(c: CallbackQuery):
    ...
    if not u.get("receipt_accepted") or not u.get("game_id_checked") or not u.get("game_password"):
        await safe_answer(c, "Սկզբում հաստատիր ID-ն, պասսվորդը և չեկը")
        return
...
async def verification(c: CallbackQuery):
    ...
    _, kind, uid_s = c.data.split(":")
    uid = int(uid_s)
    u = get_user(uid)
    labels = {"device": "Այլ սարքով հաստատում", "auth": "Authenticator", "email": "E-mail"}
    u["verification_type"] = kind
    u["verification_done"] = True  # ⭐ Сразу помечаем как выполненную
    u["status"] = f"2FA՝ {labels[kind]} — պատվերը պատրաստ է"
    await save_state()
    
    texts = {
        "device": "📱 <b>2FA — այլ սարքով հաստատում</b>\n\nՀաստատիր միայն այն մուտքը, որը դու ես սկսել արդեն մուտք գործած սարքում։",
        "auth": "🔐 <b>2FA — Authenticator</b>\n\nՕգտագործիր Authenticator-ի կոդը միայն Roblox-ի պաշտոնական էջում։ Կոդը մեզ չի ուղարկվում։",
        "email": "📧 <b>2FA — E-mail</b>\n\nՕգտագործիր E-mail կոդը միայն Roblox-ի պաշտոնական էջում։ Կոդը մեզ չի ուղարկվում։"
    }
    
    await send_client(uid, texts[kind])
    await update_admin_order(uid, admin_complete_kb(uid))
    await safe_answer(c, "2FA-ի եղանակը ընտրվեց")
...
async def order_complete(c: CallbackQuery):
    ...
    # ⭐ Проверяем - либо 2FA сделана, либо можно пропустить
    if not u.get("verification_done"):
        # ⭐ Если 2FA не сделана - просто завершаем заказ
        u["verification_done"] = True
        u["verification_type"] = "Пропущено"
    
    u["status"] = "Պատվերը ավարտված է"
    await save_state()
    await update_admin_order(uid, None)
...
async def refund_action(c: CallbackQuery):
    ...
    if len(parts) == 2:
        ...
        u["status"] = "Սպասվում են վերադարձի տվյալները"
        await save_state()
        await update_admin_order(uid, admin_refund_kb(uid))
