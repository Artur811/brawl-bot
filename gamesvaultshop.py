...
CATALOG = {
    ...
    "brawlstars": {
        "name": "⭐ Brawl Stars",
        "items": [("30 Gems", 800), ("80 Gems", 1600), ("170 Gems", 3000), ("360 Gems", 5300), ("950 Gems", 13000)],
    },
    "pubg": {
        "name": "🪂 PUBG Mobile",
        "items": [
    ...
    "brawlpass": {
        ...
        "items": [
            ...
            ("Brawl Pass Plus — аккаунт 1", 3600), ("Brawl Pass Plus — аккаунт 2", 5000),
            ("Pro Pass", 12800),
        ],
    },
}
...
async def text_router(message: Message):
    user = get_user(message.from_user.id)
    if not user.get("support_waiting"):
        return
    if not SUPPORT_CHANNEL_ID:
        return
...
async def support_channel_reply(message: Message):
    if not SUPPORT_CHANNEL_ID:
        return
    if str(message.chat.id) != str(SUPPORT_CHANNEL_ID):
        return
    ...
    if not user_id:
        match = re.search(r"ID՝ <code>(\d+)</code>", message.reply_to_message.text or message.reply_to_message.caption or "")
        if match:
...
async def payment_done(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        ...
        reply_markup=receipt_keyboard(), parse_mode="HTML",
    )
    await callback.answer()
...
# =========================================================
# RENDER HEALTH / POLLING FALLBACK
# =========================================================
