...
ROBLOX_VERIFY_IMAGES = {
    "email": "https://pbs.twimg.com/media/Eo1ZgbOVQAIgU-s.jpg",
    "authenticator": "https://devforum-uploads.s3.dualstack.us-east-2.amazonaws.com/uploads/original/4X/c/7/1/c718fc5c96c7ce8778746c7babcee27c18f50a8b.png",
    "device": "https://linuxhint.com/wp-content/uploads/2022/11/word-image-258520-6.png",
}

def blank_user():
    return {"game":None,"product":None,"price":None,"username":None,"payment":None,"order_id":None,"brawl_pass_type":None,"brawl_pass_waiting":False,"receipt_waiting_id":False,"receipt_order_message_id":None,"receipt_accepted":False,"refund_waiting":False,"refund_method":None,"refund_operator":None,"refund_details_ready":False,"refund_details":None,"game_id":None,"support_waiting":False,"verification_type":None}
...
def load_state():
    ...
    try:
        ...
        logging.exception("Could not load saved order state"); users={}

async def save_state():
    async with state_lock:
        try:
            tmp=STATE_FILE.with_suffix(".tmp")
            tmp.write_text(json.dumps(users,ensure_ascii=False),"utf-8"); tmp.replace(STATE_FILE)
        except Exception: logging.exception("Could not save order state")
...
def product_kb(game): return InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="✅ Գնել",callback_data="buy:confirm")],[InlineKeyboardButton(text="⬅️ Հետ",callback_data=f"back:game:{game}")]])
def payment_kb(): return InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="💳 Վճարել քարտով",callback_data="payment:card")],[InlineKeyboardButton(text="💵 Վճարել կանխիկ",callback_data="payment:cash")],[InlineKeyboardButton(text="⬅️ Հետ",callback_data="back:product")]])
...
def admin_verify_kb(uid):
    return InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="📧 Email-ի 2FA",callback_data=f"verify:{uid}:email")],[InlineKeyboardButton(text="🔐 Authenticator-ի 2FA",callback_data=f"verify:{uid}:authenticator")],[InlineKeyboardButton(text="📱 Մուտքի հաստատում այլ սարքով",callback_data=f"verify:{uid}:device")]])
...
async def finalize(uid,status,client_text):
    u=get_user(uid); mid=u.get("receipt_order_message_id")
    ...
    await notify(uid,client_text,main_kb()); users[uid]=blank_user(); await save_state()

def verification_text(kind):
    if kind=="email": return "🔐 <b>Roblox պահանջում է Email-ի 2-քայլ հաստատում</b>\n\n📧 Բացեք ձեր Roblox հաշվի հաստատված Email-ի մուտքային փոստը և գտեք Roblox-ից ստացված հաստատման կոդը։\n\n➡️ Կոդը մուտքագրեք միայն պաշտոնական Roblox հավելվածում կամ roblox.com-ի պաշտոնական էջում, որտեղ Roblox-ը խնդրում է այն։\n\n⚠️ <b>Կոդը երբեք մի ուղարկեք Games Vault Shop-ին, ադմինին կամ որևէ այլ անձի։</b>\n\n❗ Եթե հաստատումը չի անցնում, տեղեկացրեք ադմինին։"
    if kind=="authenticator": return "🔐 <b>Roblox պահանջում է Authenticator-ի 2FA</b>\n\n📱 Բացեք ձեր Authenticator հավելվածը և գտեք Roblox-ի համար ստեղծվող 6-նիշ կոդը։\n\n➡️ Կոդը մուտքագրեք միայն պաշտոնական Roblox հավելվածում կամ roblox.com-ի պաշտոնական էջում։\n\n⚠️ <b>Authenticator-ի կոդը երբեք մի ուղարկեք Games Vault Shop-ին, ադմինին կամ որևէ այլ անձի։</b>\n\n❗ Եթե հաստատումը չի անցնում, տեղեկացրեք ադմինին։"
    return "🔐 <b>Roblox պահանջում է մուտքի հաստատում այլ սարքով</b>\n\n📱 Բացեք Roblox հավելվածը այն հեռախոսում կամ պլանշետում, որտեղ ձեր հաշիվն արդեն մուտք գործած է։\n\n✅ Ստուգեք, որ մուտքի փորձը ձերն է, և միայն դրանից հետո հաստատեք այն։\n\n⚠️ Եթե դուք չեք կատարել այս մուտքի փորձը՝ մերժեք այն։\n\n❗ Եթե հաստատումը չի անցնում, տեղեկացրեք ադմինին։"

def verification_caption(kind): return {"email":"📧 Roblox 2FA — Email կոդ","authenticator":"🔐 Roblox 2FA — Authenticator","device":"📱 Roblox — Մուտքի հաստատում այլ սարքով"}[kind]

async def send_verification_instruction(uid,kind):
    u=get_user(uid); u["verification_type"]=kind; await save_state(); text=verification_text(kind); image=ROBLOX_VERIFY_IMAGES.get(kind)
    try:
        if image: await bot.send_photo(uid,image,caption=text,parse_mode="HTML")
        else: await bot.send_message(uid,text,parse_mode="HTML")
    except Exception:
        logging.exception("verification image/message failed"); await notify(uid,text)

@dp.message(CommandStart())
async def start(m):
    u=get_user(m.from_user.id); u["username"]=m.from_user.username; u["support_waiting"]=False; await save_state(); await m.answer(main_text(),reply_markup=main_kb(),parse_mode="HTML")
...
async def game(c):
    g=c.data.split(":",1)[1]
    if g not in CATALOG: await c.answer("❌ Սխալ խաղ։",show_alert=True); return
    u=get_user(c.from_user.id); u.update({"game":g,"product":None,"price":None,"payment":None,"brawl_pass_type":None,"brawl_pass_waiting":False,"receipt_waiting_id":False,"receipt_order_message_id":None,"receipt_accepted":False,"game_id":None,"support_waiting":False,"verification_type":None}); await save_state(); await c.message.edit_text(f"{CATALOG[g]['name']}\n\n📦 Ընտրիր անհրաժեշտ ապրանքը։",reply_markup=game_kb(g),parse_mode="HTML"); await c.answer()

@dp.callback_query(F.data.startswith("product:"))
async def product(c):
    _,g,i=c.data.split(":")
    try: name,price=CATALOG[g]["items"][int(i)]
    except (KeyError,ValueError,IndexError): await c.answer("❌ Սխալ ապրանք։",show_alert=True); return
    u=get_user(c.from_user.id); u.update({"game":g,"product":name,"price":price,"username":c.from_user.username,"support_waiting":False})
    if g=="brawlstars" and name in {"Brawl Pass","Brawl Pass+"}:
        u["brawl_pass_type"]="brawl_pass" if name=="Brawl Pass" else "brawl_pass_plus"; u["brawl_pass_waiting"]=True; await save_state(); await c.message.edit_text(f"📸 <b>Ուղարկիր screenshot-ը, որտեղ հստակ երևում է քո {escape(name)}-ի գինը։</b>\n\n👨‍💼 Screenshot-ը կստանա ադմինը և ինքը կընտրի ճիշտ գինը։",reply_markup=back_kb("back:game:brawlstars"),parse_mode="HTML"); await c.answer(); return
    await save_state(); await c.message.edit_text(f"🛒 <b>Ձեր ընտրությունը</b>\n\n🎮 Խաղ՝ <b>{escape(CATALOG[g]['name'])}</b>\n📦 Ապրանք՝ <b>{escape(name)}</b>\n💰 Գին՝ <b>{fmt(price)} ֏</b>\n\nՇարունակե՞լ պատվերը։",reply_markup=product_kb(g),parse_mode="HTML"); await c.answer()
...
async def photo(m):
    ...
    if u.get("brawl_pass_waiting"):
        ...
        sent=await bot.send_photo(ORDER_CHANNEL_ID,m.photo[-1].file_id,caption=f"📸 <b>BRAWL PASS SCREENSHOT</b>\n\n👤 @{escape(m.from_user.username or 'չկա')}\n🆔 <code>{m.from_user.id}</code>\n📦 <b>{title}</b>\n\n💰 Ընտրիր ճիշտ գինը՝ {fmt(low)} ֏ / {fmt(high)} ֏",parse_mode="HTML",reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=f"💰 {fmt(low)} ֏",callback_data=f"passprice:{m.from_user.id}:{low}"),InlineKeyboardButton(text=f"💰 {fmt(high)} ֏",callback_data=f"passprice:{m.from_user.id}:{high}")]])); u["pass_admin_message_id"]=sent.message_id; await save_state(); await m.answer("✅ Screenshot-ը ստացվեց։ Ադմինը կընտրի ճիշտ գինը։",reply_markup=main_kb()); return
    ...
    sent=await bot.send_photo(ORDER_CHANNEL_ID,m.photo[-1].file_id,caption=order_summary(m.from_user.id,u,"⏳ Չեկը սպասում է ադմինի ստուգմանը։"),parse_mode="HTML",reply_markup=admin_kb(m.from_user.id)); u["receipt_order_message_id"]=sent.message_id; await save_state(); await m.answer("💎 <b>Չեկը ստացվեց։</b> ❤️‍🔥\n\n🆔 Հիմա ուղարկիր քո Game ID / Username-ը։\n\n⚠️ Գաղտնաբառ, Email-ի կոդ կամ Authenticator-ի կոդ մի ուղարկիր։",parse_mode="HTML")

@dp.message(F.text)
async def text(m):
    u=get_user(m.from_user.id)
    if u.get("support_waiting"):
        if not SUPPORT_CHANNEL_ID: u["support_waiting"]=False; await save_state(); await m.answer("⚠️ Աջակցության ալիքը դեռ կարգավորված չէ։",reply_markup=main_kb()); return
        support_text=f"📩 <b>Նոր հաղորդագրություն աջակցությանը</b>\n\n👤 Username՝ @{escape(m.from_user.username or 'չկա')}\n🆔 Telegram ID՝ <code>{m.from_user.id}</code>\n\n💬 {escape(m.text.strip())}"
        try: await bot.send_message(SUPPORT_CHANNEL_ID,support_text,parse_mode="HTML"); u["support_waiting"]=False; await save_state(); await m.answer("✅ <b>Հաղորդագրությունը ուղարկվեց աջակցությանը։</b> ❤️‍🔥",reply_markup=main_kb(),parse_mode="HTML")
        except Exception: logging.exception("support message failed"); await m.answer("⚠️ Չհաջողվեց ուղարկել հաղորդագրությունը։")
        return
    if u.get("refund_waiting") and u.get("refund_method") in {"phone","card"} and not u.get("refund_details_ready") and (u.get("refund_operator") or u.get("refund_method")=="card"):
        u["refund_details_ready"]=True; u["refund_details"]=m.text.strip(); method="📱 Հեռախոսահամար" if u["refund_method"]=="phone" else "💳 Քարտ"; op=f"\n📞 Օպերատոր՝ <b>{escape(u.get('refund_operator') or '-')}</b>" if u.get("refund_operator") else ""; await save_state(); await notify(ADMIN_ID,f"💸 <b>Տվյալները վերադարձի համար</b>\n\n🆔 User ID՝ <code>{m.from_user.id}</code>\n📦 {escape(u.get('product') or '')}\n💰 {fmt(u.get('price') or 0)} ֏\n📌 Եղանակ՝ <b>{method}</b>{op}\n📝 Տվյալներ՝ <code>{escape(m.text.strip())}</code>",admin_kb(m.from_user.id,u.get("receipt_accepted",False),True)); await m.answer("✅ Տվյալները ստացվեցին։ Սպասիր վերադարձի հաստատմանը։"); return
    if u.get("receipt_waiting_id") or (u.get("payment") in {"receipt_sent","receipt_accepted"} and u.get("receipt_order_message_id")):
        ...
        if not game_id: await m.answer("⚠️ Ուղարկիր ճիշտ Game ID / Username։"); return
        u["game_id"]=game_id; u["receipt_waiting_id"]=False; await save_state(); caption=order_summary(m.from_user.id,u,"⏳ Չեկը սպասում է ադմինի ստուգմանը։")
        try: await bot.edit_message_caption(chat_id=ORDER_CHANNEL_ID,message_id=u["receipt_order_message_id"],caption=caption,parse_mode="HTML",reply_markup=admin_kb(m.from_user.id,u.get("receipt_accepted",False),u.get("refund_details_ready",False)))
        except Exception: logging.exception("Could not update order with customer Game ID")
        await m.answer("✅ <b>Game ID / Username-ը ստացվեց։</b>\n\n⏳ Վճարումը ստուգվում է, և պատվերը մշակվում է։",parse_mode="HTML"); return
    await m.answer("ℹ️ Եթե ուզում ես ուղարկել պատվերի տվյալները, նախ ուղարկիր չեկի նկարը։",reply_markup=main_kb())
...
async def receipt(c):
    ...
    if action=="reject": await finalize(uid,"❌ Չեկը մերժված է — Դոնաթը չի հաջողվել.","❌ <b>Դոնաթը չի հաջողվել.</b>\n\nՎճարման չեկը մերժվել է։ Եթե կարծում ես, որ սխալ է, կապվիր աջակցությանը."); await c.answer("❌ Չեկը մերժվեց։"); return
    if action=="accept":
        u["receipt_accepted"]=True; u["payment"]="receipt_accepted"; u["receipt_order_message_id"]=c.message.message_id; await save_state()
        if u.get("game")=="roblox":
            await c.message.edit_caption(caption=order_summary(uid,u,"✅ Չեկը հաստատված է։ Ընտրիր Roblox-ի ստուգման տեսակը։"),parse_mode="HTML",reply_markup=admin_verify_kb(uid)); await notify(uid,"✅ <b>Չեկը հաստատվեց։</b>\n\n⏳ Ադմինը կընտրի Roblox-ի պահանջվող հաստատման տեսակը։")
        else:
        ...
        await c.answer("✅ Չեկը հաստատվեց։"); return
    if action=="confirm":
    ...
    if action=="refund":
        u["refund_waiting"]=True; u["refund_method"]=None; u["refund_operator"]=None; u["refund_details_ready"]=False; await save_state(); await notify(uid,"⚠️ <b>Դոնաթը չի հաջողվել։</b>\n\n💸 Ընտրիր, թե որտեղ պետք է կատարվի վերադարձը։",refund_method_kb()); await c.answer("💸 Հաճախորդին ուղարկվեց վերադարձի ընտրությունը։"); return
...
async def verify(c):
    if not await admin_only(c): return
    _,uidtxt,kind=c.data.split(":"); uid=int(uidtxt)
    if kind not in ROBLOX_VERIFY_IMAGES: await c.answer("❌ Սխալ ստուգման տեսակ։",show_alert=True); return
    u=get_user(uid)
    if not u.get("receipt_accepted"): await c.answer("⚠️ Նախ հաստատիր չեկը։",show_alert=True); return
    u["verification_type"]=kind; await save_state()
    try: await c.message.edit_caption(caption=order_summary(uid,u,f"🔐 Ընտրված է՝ {verification_caption(kind)}"),parse_mode="HTML",reply_markup=admin_kb(uid,True,u.get("refund_details_ready",False)))
    except Exception: logging.exception("Could not update verification status")
    await send_verification_instruction(uid,kind); await c.answer("✅ Հաճախորդին ուղարկվեց համապատասխան ուղեցույց։")
...
async def contact(c):
    u=get_user(c.from_user.id); u["support_waiting"]=True; await save_state(); await c.message.edit_text("📩 <b>Կապվեք մեզ հետ</b>\n\n💬 Գրիր քո հաղորդագրությունը, և մենք կստանանք այն։",reply_markup=back_kb("back:main"),parse_mode="HTML"); await c.answer()

@dp.callback_query(F.data=="back:main")
async def bmain(c):
    u=get_user(c.from_user.id); u["support_waiting"]=False; await save_state(); await c.message.edit_text(main_text(),reply_markup=main_kb(),parse_mode="HTML"); await c.answer()
...
async def main():
    load_state(); app=web.Application(); app.router.add_get("/",health); app.router.add_get("/health",health); runner=web.AppRunner(app); await runner.setup(); await web.TCPSite(runner,"0.0.0.0",PORT).start(); await bot.delete_webhook(drop_pending_updates=True); await dp.start_polling(bot)
